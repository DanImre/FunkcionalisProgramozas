...
void titkosit(char bemenet[], char kimenet[]) {
  ...
  //  printf("%d\n", hossz);
   ...
   /* for(i=0; i<db; ++i)
    ...
    while (bemenet[i] != '\0')
    {
        ...
        if(bemenet[i] == ' ' || bemenet[i] == '\0')
        {
            ...
            while (j > elozoi)
            {
                forditottSzavakSpaceel[forditottSzavakSpaceelIndex] = bemenet[j];
                --j;
                ++forditottSzavakSpaceelIndex;
            }
            elozoi = i;
   ...
   /* printf("actual size : %d\n", sizeof(forditottSzavakSpaceel)/sizeof(char));
    ...
    kimenet[hossz+db-spacek];
    for (size_t kk = 0; kk < (hossz-spacek) && forditottSzavakSpaceel[kk]!='\0'; kk++)
    {
        kimenet[db + kk] = forditottSzavakSpaceel[kk];
    }
   // printf("%s\n", kimenet);
